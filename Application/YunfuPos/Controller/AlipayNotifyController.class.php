...
class AlipayNotifyController extends Controller {
	...
	public function notify() {
		...
		if($verify_result) {//验证成功
			...
			//商户订单号
			file_put_contents ("log.txt", date ( "Y-m-d H:i:s" ). var_export($_POST,true). "\r\n", FILE_APPEND );
			...
			$order = M('order')->where(array('uid'=>$uid,'out_trade_no'=>$out_trade_no,'trade_no'=>$trade_no))->save($data);
				
			//判断是否在商户网站中已经做过了这次通知返回的处理
			//如果没有做过处理，那么执行商户的业务程序
			//如果有做过处理，那么不执行商户的业务程序
			//file_put_contents ("log.txt", date ( "Y-m-d H:i:s" ) . " success ". "\r\n", FILE_APPEND );
			...
			//验证失败
			//file_put_contents ("log.txt", date ( "Y-m-d H:i:s" ) . " fail ". "\r\n", FILE_APPEND );
			echo "fail";
