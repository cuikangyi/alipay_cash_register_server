...
class IndexController extends AgentController {
    ...
    public function chartData(){
        ...
        //$field = array('substr(gmt_create,6,5)'=>'day','COUNT(Orders.id)'=>'count','SUM(total_fee)'=>'total_fee');
        $field = 'LEFT(gmt_create,10) as days,COUNT(Orders.id) as nums,SUM(total_fee) as total_fee';
        $list = $Model->field($field)->where($where)->group("LEFT( gmt_create, 10)")->order("LEFT( gmt_create, 10) desc")->limit(10)->select();
        $total_count = $Model->where($where)->count('Orders.id');
        ...
        $day_count = $Model->where($where)->count('Orders.id');
