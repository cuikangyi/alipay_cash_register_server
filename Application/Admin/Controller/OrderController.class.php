...
class OrderController extends AdminController
{
   ...
   /* private function ExportExcel(){
        ...
        if(I('time_type') == 'gmt_payment'){
            $where['gmt_payment'] =   array('between',array($start_time,$end_time));
        }else{
            $where['gmt_create'] =   array('between',array($start_time,$end_time));
        }
        ...
        if($start_fee && $end_fee){
            ...
            $where['total_fee'] = array('egt',$start_time);
        }elseif($end_fee){
            $where['total_fee'] = array('elt',$end_fee);
        }
        ...
        $OrdersData= $model->where($where)->order('id desc')->select();  //查询数据得到$OrdersData二维数组
        ...
        for($i=0;$i<count($OrdersData)-1;$i++){
            ...
            $pay_status = $this->getPayStatus($OrdersData[$i]['result_code'],$OrdersData[$i]['trade_status']);
